...
def Function_simulation(Sys_Pc_vcm, Sys_Pc_pzt, Sys_Cd_vcm, Sys_Fm_vcm, Sys_Cd_pzt, Sys_Fm_pzt,
                        Ts, Mr_f, iter, fan_noise_type="random", fan_freq_hz=None,
                        fan_amp=None, fan_scale=0.01, fan_phase=None,
                        output_suffix=None):
    ...
    #   fan_noise_type = "sinusoidal" -> deterministic sinusoidal fan disturbance
    fan_noise_type = str(fan_noise_type).lower()
    if fan_noise_type in ("sine", "sin", "deterministic"):
        fan_noise_type = "sinusoidal"
    if fan_noise_type not in ("random", "sinusoidal"):
        raise ValueError('fan_noise_type must be either "random" or "sinusoidal"')

    if fan_freq_hz is None:
        fan_freq_hz = np.array([5072, 5850, 7670, 9200], dtype=float)
    else:
        fan_freq_hz = np.asarray(fan_freq_hz, dtype=float)

    if fan_amp is None:
        fan_amp = np.array([0.5, 2.0, 1.5, 2.5], dtype=float)
    else:
        fan_amp = np.asarray(fan_amp, dtype=float)

    if fan_phase is None:
        fan_phase = np.zeros_like(fan_freq_hz, dtype=float)
    else:
        fan_phase = np.asarray(fan_phase, dtype=float)

    if not (len(fan_freq_hz) == len(fan_amp) == len(fan_phase)):
        raise ValueError("fan_freq_hz, fan_amp, and fan_phase must have the same length")
    ...
    for n in range(round(Tsim/Tsc)+1):
        ...
        elif fan_noise_type == "sinusoidal":
            # Deterministic fan disturbance.  The values in fan_amp are sinusoid
            # amplitudes, not DC gains of the original second-order filters.
            t_now = n * Tsc
            ddp = fan_scale * np.sum(
                fan_amp * np.sin(2 * np.pi * fan_freq_hz * t_now + fan_phase)
            )
            dp[n] = ddp
    ...
    sim_result['fan_noise_type'] = fan_noise_type
    sim_result['fan_freq_hz'] = fan_freq_hz
    sim_result['fan_amp'] = fan_amp
    sim_result['fan_scale'] = fan_scale

    output_name = "res" + str(iter)
    if output_suffix is not None and str(output_suffix) != "":
        output_name += "_" + str(output_suffix)
    output_path = utils.get_sim_path(output_name + ".pkl")
...
def Function_simulation_clean(Sys_Pc_vcm, Sys_Pc_pzt, Sys_Cd_vcm, Sys_Fm_vcm, Sys_Cd_pzt, Sys_Fm_pzt,
                              Ts, Mr_f, iter, fan_freq_hz=None, fan_amp=None,
                              fan_scale=0.01, fan_phase=None, output_suffix=None):
    """
    Backward-compatible wrapper for deterministic sinusoidal fan disturbance.
    New code should call Function_simulation(..., fan_noise_type="sinusoidal") directly.
    """

    return Function_simulation(
        Sys_Pc_vcm,
        Sys_Pc_pzt,
        Sys_Cd_vcm,
        Sys_Fm_vcm,
...
def run_case(case_id, fan_noise_type="random", fan_freq_hz=None, fan_amp=None,
             fan_scale=0.01, fan_phase=None, output_suffix=None):
    """
    ...
    ----------
    case_id : int
        Benchmark case ID from 1 to 9.
    fan_noise_type : {"random", "sinusoidal"}
        "random" uses the original stochastic fan-disturbance filters.
        "sinusoidal" uses deterministic sinusoidal fan disturbance.
    output_suffix : str or None
        Optional suffix for the saved pickle file. For example, output_suffix="sine"
        saves res2_sine.pkl instead of res2.pkl for case 2.
    """

    if case_id < 1 or case_id > 9:
        raise ValueError("case_id must be between 1 and 9")

    print(f"Running case {case_id} with fan_noise_type='{fan_noise_type}'...")
...
if __name__ == '__main__':
    ...
    OUTPUT_SUFFIX = None           # example: "random" or "sine" to avoid overwriting res2.pkl
