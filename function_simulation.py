...
def Function_simulation_clean(Sys_Pc_vcm,Sys_Pc_pzt,Sys_Cd_vcm,Sys_Fm_vcm,Sys_Cd_pzt,Sys_Fm_pzt,Ts,Mr_f,iter):
    # Define sim_result as a dictionary
    sim_result = {
    'time': None,
    'freq': None,
...
#    Tsim=1.1                         # End of simulation time
    ...
    for n in range(round(Tsim/Tsc)+1):
        ...
        # FAN-induced disturbance as deterministic sinusoids
        t_now = n * Tsc

        # fan_freq_hz = np.array([2200, 2937, 3300, 3545, 3980, 4220, 4380, 5072, 5370, 5850, 6660, 7670, 9200])
        # fan_amp = np.array([0.6, 0.3, 1.0, 0.5, 0.3, 1.0, 1.0, 0.5, 2.0, 15.0, 10.0, 1.5, 2.5])
        fan_freq_hz = np.array([5072, 5370, 5850, 7670, 9200])
        fan_amp = np.array([0.5, 2.0, 15.0, 1.5, 2.5])

        # Optional: scale amplitudes if too large
        fan_scale = 0.01

        ddp = fan_scale * np.sum(
            fan_amp * np.sin(2 * np.pi * fan_freq_hz * t_now)
            )
